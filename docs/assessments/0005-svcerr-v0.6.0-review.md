...
`go.mod` declares `go 1.25.0` for a zero-dependency module whose newest stdlib usage is `http.NewResponseController` (Go 1.20, tests only). Two external reviews independently ran the suite on Go 1.23 successfully. A high floor costs adopters for nothing. Lower the directive to a verified minimum and add oldest+latest Go versions to the CI matrix so the floor is enforced rather than aspirational.
